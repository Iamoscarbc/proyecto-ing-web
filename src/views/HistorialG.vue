    ...
    <div class="table-responsive">
      <base-table class="table align-items-center table-flush"
        ...
        <template slot-scope="{row}">
            ...
            <td>
                <badge class="badge-dot mr-4" :type="`bg-success`">
                    <span class="status" style="text-transform:capitalize" v-if="row.vueltaOrigen == null">Solo Ida</span>
            ...
            <td>
                <badge class="badge-dot mr-4" :type="`bg-success`">
                    <span class="status" style="text-transform:capitalize" v-if="row.comprador != null">{{row.comprador}}</span>
                    <span class="status" style="text-transform:capitalize" v-else> - </span>
                </badge>
            </td>
            <td>
                <badge class="badge-dot mr-4" :type="`bg-success`">
            ...
            <td>
                <badge class="badge-dot mr-4" :type="`bg-success`">
                    <span class="status" style="text-transform:capitalize" v-if="row.vueltaOrigen == null">-</span>
                    <span class="status" style="text-transform:capitalize" v-else>{{row.vueltaOrigen}}</span>
                </badge>
            </td>
            <td>
                <badge class="badge-dot mr-4" :type="`bg-success`">
                    <span class="status" style="text-transform:capitalize" v-if="row.vueltaDestino == null">-</span>
...
// import Modal1 from '../Modal/Modal1'
  export default {
    ...
    beforeCreate() {
        ...
        let _this = this;
        if(token){
            this.$store.dispatch('get_pasajes_general',{
                jwt: token
            }).then(response => {
